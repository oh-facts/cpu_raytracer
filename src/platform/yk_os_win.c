...
b8 yk_input_is_key_tapped(struct YkKeyState *state, u32 key)
{
    return state->_cur[key] && !state->_old[key];
}

b8 yk_input_is_key_held(struct YkKeyState *state, u32 key)
{
    return state->_cur[key];
}

b8 yk_input_is_key_released(struct YkKeyState *state, u32 key)
{
    return !state->_cur[key] && state->_old[key];
}

b8 yk_input_is_click(struct YkMouseClickState *state, YK_MOUSE_BUTTON button)
{
    return state->_cur[button] && !state->_old[button];
}

v2 yk_input_mouse_mv(struct YkMousePosState *state)
{
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_INPUT:
    {
        struct YkWindow *win = (struct YkWindow *)GetWindowLongPtrW(hwnd, GWLP_USERDATA);
        ...
        if (raw->header.dwType == RIM_TYPEKEYBOARD)
        {
            ...
            const b8 isE0 = ((flags & RI_KEY_E0) != 0);
            const b8 isE1 = ((flags & RI_KEY_E1) != 0);
            ...
            */
            ...
            const b8 wasUp = ((flags & RI_KEY_BREAK) != 0);
    ...
    */
    case WM_CREATE:
    {
        struct YkWindow *win = (struct YkWindow *)((CREATESTRUCT *)lParam)->lpCreateParams;
...
void yk_window_update(struct YkWindow *window)
{
    for (u32 i = 0; i < 256; i++)
    {
...
void yk_free_window(struct YkWindow *window)
{
